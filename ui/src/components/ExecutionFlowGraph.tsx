...
type EdgeData = GraphEdgeSpec & { highlight: HighlightRole };
 ...
 * re-render.
 *
 * NOT wrapped in `withSelection` as well. Selection would give a node a
 * highlight state, but this view has nothing to do with a selected node — no
 * side panel opens, no table row follows — so it would be a visual state the
 * reader could enter and get nothing from. Kept out until there is something for
 * it to drive.
 *
...
/**
 * The graph renderer. Pan/zoom only — see the layout note in the component for
 * why no layout is registered at all.
 */
const PanZoomGraph = withPanZoom()(GraphComponent);
...
export function EntityGraph({
  ...
  useEffect(() => {
    ...
    const model: Model = {
      ...
      edges: spec.edges.map((e) => ({
        ...
        data: { ...e, highlight: roleOf(e.id, highlight, false) } satisfies EdgeData,
      })),
    };
  ...
  }, [controller, spec, traceId, harvestDraggedPositions, highlightKey]);
...
export function ExecutionFlowGraph({ traceId }: { traceId: string }) {
  ...
  return (
    ...
    }) ?? <EntityGraph spec={spec} traceId={traceId} />
  );
}
 ...
 * of ADR-0027 restated for a graph:
 ...
 *                              "checked, no sources");
 *   - selected, derived-empty → the real verdict "originates here" (ADR-0027 D3),
...
export function LineageGraph({
  ...
  /** The trace's coverage (ADR-0027 D6), so a prefix answer says so. */
      ...
      notices={
        <>
          {/* NOTHING SELECTED. An instruction, not a verdict — the graph below is
              ...
              only place a selection can be made (the graph's nodes are not click
              targets — see the drag note in DraggableKindColouredNode). */}
          {lineage.selectedNodeId === null && (
            <Alert
              variant="info"
              isInline
          ...
          {/* NOT YET DERIVED. The eventual-consistency window (ADR-0027): legs DO
          ...
          {/* DERIVED AND EMPTY — a REAL answer (ADR-0027 D3), and the one state a
          ...
          {/* THE TRACE-LEVEL PREFIX (ADR-0027 D6). Reused verbatim rather than
