...
export function HighlightLegend({
  ...
  return (
    ...
    >
      <div style={{ color: '#888', textTransform: 'uppercase', fontSize: '0.7rem', marginBottom: '0.3rem' }}>
        Highlighted sets
      </div>
      {pins.map((p) => (
        <div key={p.key} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.1rem 0' }}>
          ...
          <span style={{ flex: 1, color: '#c9c9c9', overflow: 'hidden', textOverflow: 'ellipsis' }} title={p.label}>
            {p.label}
          </span>
