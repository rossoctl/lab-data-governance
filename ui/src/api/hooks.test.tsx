...
describe('useDataLineage', () => {
  ...
  it('fetches the trace-scoped data-lineage resource and keys the legs by (interaction_id, leg_type)', async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({ legs: [LEG, { ...LEG, leg_type: 'response', lineage: null }] }),
    });
    ...
    expect(result.current.data?.get('i1:request')).toEqual(LEG.lineage);
    // A leg present with lineage=null is a real "not yet computed" entry, and
    // must stay distinguishable from an absent leg.
    expect(result.current.data?.has('i1:response')).toBe(true);
    expect(result.current.data?.get('i1:response')).toBeNull();
    expect(result.current.data?.has('i2:request')).toBe(false);
  });
