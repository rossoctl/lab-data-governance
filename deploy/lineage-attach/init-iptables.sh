...
# OUTBOUND_PORTS_INCLUDE (redirect mode only) — the include-only allowlist, the
# INVERSE of OUTBOUND_PORTS_EXCLUDE. When non-empty, PROXY_OUTPUT redirects ONLY
# these dports to the outbound listener and RETURNs everything else DIRECT
# (terminal RETURN instead of the terminal catch-all REDIRECT). Fail-safe: a port
# not on the list stays direct (correct) rather than being handed to a proxy that
# cannot parse it (broken) — exactly the trade the lineage proxy-sidecar wants
# (redirect only the HTTP hops it can parse: A2A 8080 + MCP 8000; leave the
# opaque legs — Postgres, SMTP, the TLS LLM tunnel — untouched). Additive: the
# existing exclude/denylist path is unchanged, and the two are mutually exclusive
# (opposite models — refused together below). No effect in enforce-redirect mode,
# which has its own fail-closed capture design.
OUTBOUND_PORTS_INCLUDE="${OUTBOUND_PORTS_INCLUDE:-}"
if [ -n "${OUTBOUND_PORTS_INCLUDE}" ] && [ -n "${OUTBOUND_PORTS_EXCLUDE}" ]; then
  echo "ERROR: OUTBOUND_PORTS_INCLUDE and OUTBOUND_PORTS_EXCLUDE are opposite models (allowlist vs denylist) — set at most one." >&2
  exit 1
fi
# Validate the allowlist here too, not only in the generator: this script is
# independently runnable (the proxy-init container env, a hand-edited manifest, a
# future operator override), and each token is interpolated straight into
# `--dport ${port}` below. A malformed value would word-split extra tokens into
# the iptables command or build a chain that fails to redirect the intended hops
# — the opposite of the fail-safe intent. A comma-separated list of ports
# (1-65535, no leading zeros — iptables reads `010` as octal); empty is fine.
case "${OUTBOUND_PORTS_INCLUDE}" in
  "") ;;
  *[!0-9,]*|,*|*,|*,,*)
    echo "ERROR: OUTBOUND_PORTS_INCLUDE='${OUTBOUND_PORTS_INCLUDE}' is not a comma-separated list of ports." >&2
    exit 1 ;;
  *)
    for _oip in $(echo "${OUTBOUND_PORTS_INCLUDE}" | tr ',' ' '); do
      case "${_oip}" in
        0*) echo "ERROR: OUTBOUND_PORTS_INCLUDE port '${_oip}' has a leading zero (iptables reads it as octal)." >&2; exit 1 ;;
      esac
      if [ "${_oip}" -lt 1 ] || [ "${_oip}" -gt 65535 ]; then
        echo "ERROR: OUTBOUND_PORTS_INCLUDE has '${_oip}', which is not a port (1-65535)." >&2; exit 1
      fi
    done ;;
esac

# The include-only allowlist is an EGRESS-ONLY interception: it exists for the
# auth-free lineage proxy-sidecar, which binds only its forward/transparent
# listeners (8081/8082) and health (9091) — it has NO inbound listener on
# INBOUND_PROXY_PORT (that is the ENVOY inbound port, absent in proxy-sidecar
# mode). redirect mode's default is symmetric: it ALSO redirects inbound (nat
# PREROUTING) and ztunnel's ambient inbound delivery (the Rule-2 DNAT) to
# INBOUND_PROXY_PORT. For a proxy-sidecar that port is dead, so leaving those on
# would REDIRECT/DNAT every inbound request to a closed port — the app becomes
# unreachable inbound and nothing serves. So when the allowlist is in force we
# skip inbound interception entirely: the app's inbound stays DIRECT (reachable
# on its own port), and lineage still sees each hop once, on the CALLER's egress.
# This keeps the include-only path self-consistent — an outbound-only capture, as
# ADR-0033 describes ("redirect only A2A/MCP", the OUTBOUND allowlist).
EGRESS_ONLY=0
[ -n "${OUTBOUND_PORTS_INCLUDE}" ] && EGRESS_ONLY=1
...
# POD_IP is only needed by redirect mode's ambient inbound DNAT (Rule 2). The
# EGRESS_ONLY (allowlist) path skips that rule entirely — the only redirect-mode
# consumer of POD_IP — so it does not need it. enforce-redirect does no DNAT
# either. So require POD_IP only for a redirect mode that will actually install
# the DNAT (i.e. NOT egress-only).
if [ "${MODE}" = "redirect" ] && [ "${EGRESS_ONLY}" != "1" ] && [ -z "${POD_IP}" ]; then
...
# Rules 1-2 handle ztunnel's AMBIENT INBOUND delivery (mesh HBONE terminated
# locally and re-originated through the OUTPUT chain), sending it to the inbound
# listener. In EGRESS-ONLY mode there is no inbound listener (see EGRESS_ONLY
# above), so both are skipped — mesh inbound stays direct like plain inbound.
if [ "${EGRESS_ONLY}" != "1" ]; then
...
fi  # end EGRESS_ONLY guard around Rules 1-2 (ambient inbound delivery)
...
# --- Terminal rule: redirect the remaining outbound TCP to the proxy listener ---
# Two shapes, chosen by OUTBOUND_PORTS_INCLUDE:
#   * allowlist (INCLUDE set) — REDIRECT only the listed dports, then a terminal
#     RETURN so every OTHER port passes DIRECT. This is the fail-safe model: a
#     port left off the list is not sent to a proxy that would break it. Used by
#     the auth-free lineage proxy-sidecar (only A2A 8080 + MCP 8000 are HTTP hops
#     it can parse; Postgres/SMTP/the TLS LLM tunnel stay direct).
#   * denylist / default (INCLUDE unset) — the historical catch-all: REDIRECT all
#     remaining TCP, with OUTBOUND_PORTS_EXCLUDE (above) carving out ports.
if [ -n "${OUTBOUND_PORTS_INCLUDE}" ]; then
  for port in $(echo "${OUTBOUND_PORTS_INCLUDE}" | tr ',' ' '); do
    echo "Including outbound port ${port} for redirection (allowlist mode)"
    ${IPT} -t nat -A PROXY_OUTPUT -p tcp --dport "${port}" -j REDIRECT --to-port "${PROXY_PORT}"
  done
  # Everything not on the allowlist passes through direct.
  ${IPT} -t nat -A PROXY_OUTPUT -j RETURN
else
  # --- Catch-all: redirect remaining outbound TCP to the outbound listener ---
...
#
# EGRESS-ONLY (OUTBOUND_PORTS_INCLUDE set): skip this whole section. The catch-all
# below REDIRECTs every inbound app port to INBOUND_PROXY_PORT — the ENVOY inbound
# listener, which the auth-free proxy-sidecar does NOT bind — so installing it
# would send every inbound request to a dead port and make the app unreachable.
# Leaving inbound direct is correct for the egress-only lineage proxy: the app
# serves on its own port, and each hop is captured on the caller's egress.
if [ "${EGRESS_ONLY}" = "1" ]; then
  echo "Skipping inbound interception (egress-only: OUTBOUND_PORTS_INCLUDE set; no inbound listener to redirect to)"
  exit 0
fi
