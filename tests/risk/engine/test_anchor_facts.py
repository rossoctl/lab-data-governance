...
def test_outbound_internal_destination_has_unknown_trust_level() -> None:
    ...
    assert dest["data_destination_trust_level"] == "UNKNOWN", (
        "internal carries no guessed TRUSTED_* value — the trust level is "
        "derived from the category, and the MVP whitelist implies none for "
        "internal, so it reads UNKNOWN"
    )
