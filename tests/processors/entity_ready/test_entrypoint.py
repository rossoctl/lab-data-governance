"""Entrypoint behaviour for the P-entity-ready consumer (issue #121, ADR-0027).

Boots ``python -m data_governance.processors.entity_ready`` as a subprocess and
asserts the process-boundary contract (exit codes, clean SIGTERM), mirroring the
P-classification entrypoint tests: the entrypoint reads ``DATABASE_URL``, runs the
defence-in-depth schema-version check (ADR-0002), and drives the shared
wake-driven drain loop until SIGINT/SIGTERM. The entity-ready stream has no
readiness gate and no per-item model, so — unlike P-classification — there is
nothing to construct at startup.
"""
...
def _spawn(dsn: str | None) -> subprocess.Popen[str]:
    env = {
        ...
        # does not collide on the default 9093.
        "ENTITY_READY_METRICS_PORT": "0",
    }
    if dsn is not None:
        env["DATABASE_URL"] = dsn
    return subprocess.Popen(
        [sys.executable, "-m", "data_governance.processors.entity_ready"],
        env=env,
...
def test_entrypoint_exits_3_on_schema_mismatch(migrated_dsn: str) -> None:
    ...
    P-classification entry points: a ``SchemaVersionMismatch`` exits with code 3
    specifically (not merely non-zero), and the actionable error names both the
    stale DB revision and the compiled head. This is the CrashLoopBackOff signal
    k8s surfaces."""
...
def test_entrypoint_exits_nonzero_when_alembic_version_missing(pg_dsn: str) -> None:
    ...
    (ADR-0002). This is the "init container forgotten" failure mode."""
    proc = _spawn(pg_dsn)
    rc = proc.wait(timeout=10)
    stderr = proc.stderr.read() if proc.stderr else ""
