...
ADR-0026 and the openai_agents v1.4.1 span reference.
...
Per ADR-0026 the LLM and tools are *one-sided* observations: only the caller
...
and the `inferred` boolean, never the display string (ADR-0026 "Inferred
...
def test_one_observed_agent_rest_inferred(canonical_trace_spans):
    ...
    # detected_from and the inferred boolean agree — ADR-0026 "Inferred identity
