...
to `tool:delegate_to_*` peers: per ADR-0026 Step 2.d rule 4 each delegation call
...
#
# The two delegations are `agent → agent` (ADR-0026 Step 2.d rule 4), not calls
...
# The full interaction sequence, sorted by `order` alone (ADR-0026 consumer
...
def test_interactions_are_bidirectional(multi_agent_delegation_trace_spans):
    ...
    ADR-0026 Step 2.c adds bidirectional Black edges for each call site, so the
...
def test_delegation_response_leg_anchors_on_callee_span(
    ...
    the caller's `delegate_to_*` call-site span (ADR-0026 Step 3.c / Step 2.d
    rule 4).
...
def test_order_is_dense_unique_ordinal(multi_agent_delegation_trace_spans):
    ...
    ADR-0026: consumers sort by `order` alone, so it must be total and unique.
    """
    result = extract(multi_agent_delegation_trace_spans)
...
def test_interaction_order_is_exact_sequence(multi_agent_delegation_trace_spans):
    ...
    Pins the exact execution-order sequence (ADR-0026 Step 3.b): consumers sort by
