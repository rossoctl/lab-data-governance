...
downstream agent — so ADR-0026 Step 2.d rule 4 collapses them: the observed
...
Per ADR-0026 the four agents are the `observed` entities; the six tools and
...
`detected_from` — per ADR-0026 ("Natural-key prefixes are part of the public
...
EXPECTED_ENTITIES = {
    ...
    # NOTE: the two `delegate_to_*` "tools" are NOT here. Per ADR-0026 Step 2.d
