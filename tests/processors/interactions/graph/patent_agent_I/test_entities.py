...
transport):
  ...
  * ONE agent, `patent-assistant`, which is *inferred* rather than observed:
    ...
    ADR-0026 Step 2.c case 4 (`infer_agent_from_bare_leaf_llms`) an agent node is
...
assert on the stable, ADR-0026-sanctioned signals — `natural_key`, `inferred`,
...
EXPECTED_ENTITIES = {
    ...
    # (ADR-0026 Step 2.c case 4, `infer_agent_from_bare_leaf_llms`).
