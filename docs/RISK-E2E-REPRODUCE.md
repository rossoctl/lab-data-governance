...
|---|---|---|
| `rossoctl/lab-data-governance` | `risk-dev-L` | this repo — DG service + risk engine + risk UI (the `risk` line merged with the engine-integration line, PR #177 and PR #198) |
| `s-and-p-team/cortex` | `lineage-dev-R` | the lineage sidecar plugin (#761), the attach kit (#852), and the travel demo (`authbridge/demos/lineage-travel/`) |
...
## 1 · Data-governance side (this repo, branch `risk-dev-L`)

```sh
git switch risk-dev-L
...
- PR #198's page is folded in from its open branch; when that PR changes, re-merge.
