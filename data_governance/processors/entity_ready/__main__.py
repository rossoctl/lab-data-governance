"""P-entity-ready consumer entry point — ``python -m data_governance.processors.entity_ready``.

Reads ``DATABASE_URL`` (matching the migrate CLI / receiver / P-interactions /
P-classification convention), runs the defence-in-depth schema-version check,
then drives the shared wake-driven drain loop (:func:`driver.run` — a ``LISTEN``
notification on ``dg_entity_ready`` or the poll backstop wakes each drain) until
...
exactly, differing only in the module names, the logger, the metrics-port env
override, and the absence of a model load — the entity-ready stream has no
readiness gate and no per-item model, so there is nothing to construct at startup
(ADR-0027). The downstream governance consumer (risk/lineage/PDP) is future work;
until it lands, delivery is the drain's metric increment, so ``run`` is driven
with no injected observer.

Before processing any entities the entry point runs the schema-version check
...
the migrate init container is the primary ordering mechanism but the consumer
...
# Distinct from the receiver's 9090, the interactions processor's 9091, and the
# classification processor's 9092 so a co-located deployment doesn't collide on
# the metrics port. Overridable via ENTITY_READY_METRICS_PORT (0 = ephemeral,
# used by tests).
...
def _int_env(name: str, default: int) -> int:
    ...
    Lets the consumer's metrics port be overridden from outside the process (e.g.
    so integration tests can run multiple processors without colliding).
...
def main() -> int:
    ...
    log = logging.getLogger("data_governance.processors.entity_ready")
    ...
    # Defence-in-depth schema-version check (issue #10, ADR-0002). The consumer
    ...
    metrics_server = MetricsServer(
        port=_int_env("ENTITY_READY_METRICS_PORT", DEFAULT_METRICS_PORT),
        registry=metrics._registry,
    )
    metrics_server.start()
    log.info(
        "entity-ready consumer Prometheus /metrics on %s:%d",
        metrics_server.host,
        metrics_server.port,
    )
