...
def _reconstruct_two_component_transport_chains(
    ...
    """ADR-0026 Step 3.b for observed transport chains — reconstruct one
    ...
    for chain in _observed_transport_chains(graph):
        ...
        if len(endpoint_by_entity) > 2:
            entity_graph.notes.append(
                ...
                "entities; >2-way transport reconstruction is deferred (ADR-0026)"
            )
            continue
...
def build_entity_graph(graph: BaseGraph, spans_by_id: dict[str, Span]) -> EntityGraph:
    ...
    **between two Blue components** yields a directed S→D request entity edge
    ...
    # Dropping Teal is the entity cut (ADR-0026 Step 3.a "drop the Teal nodes"):
    ...
    # --- Observed transport chains (Step 2.a / ADR-0026 Step 3.b) ---------
...
def _order_execution_walk(
    ...
    """ADR-0026 / spec **Step 3.b point 2** — assign each interaction edge a
...
def combine_identical_entities(
    ...
    """ADR-0026 / spec **Step 3.d semantic combine** — combine entity-graph nodes
